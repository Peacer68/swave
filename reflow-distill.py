...
from data import AudioDataset, MelSpectrogramFixed, MatchDataset, PhaseFixed
...
def run_training(rank, config, args):
    ...
    config.training_config.logdir = f"{config.training_config.logdir}-distillstep{config.training_config.distill_step}-ss{config.training_config.step_sampler}-noise{config.training_config.noise_scale}-tignore{config.training_config.t_ignore}-lr{config.training_config.lr}-weightdecay{config.training_config.weight_decay}-bsz{config.training_config.batch_size}-warmup{config.training_config.warm_up}"
    logger = Logger(config, rank=rank)

    show_message('Initializing model...', verbose=args.verbose, rank=rank)
    model = WaveGrad(config)
    model.load_state_dict(torch.load(config.training_config.teacher_ckpt_path, map_location=torch.device("cpu"))["model"])
    model.cuda()
    ...
    optimizer = torch.optim.Adam(params=model.parameters(), lr=config.training_config.lr, weight_decay=config.training_config.weight_decay)
    if config.training_config.continue_training:
        ...
        model, optimizer, iteration, epoch_start = logger.load_latest_checkpoint(model, optimizer)
    else:
        iteration = 0
        epoch_start = 1
    ...
    train_dataset = MatchDataset(config)
    print(f"The length of Data: {len(train_dataset)}")
    ...
    try:
        for epoch in range(epoch_start, config.training_config.n_epoch+1):
            ...
            for batch in (
                ...
                model.zero_grad()
                y_0, y_1, y_0_pred = batch["y_0"].squeeze(0).cuda(), batch["y_1"].squeeze(0).cuda(), batch["y_0_pred"].squeeze(0).cuda()

                mels = mel_fn(y_0) # (channels, n_mels, time)
                    
                if config.training_config.use_fp16:
                    with torch.cuda.amp.autocast():
                        loss = (model if args.n_gpus == 1 else model.module).compute_loss_reflow(mels, y_0_pred, y_1, phase_fn=phase_fn, mel_fn=mel_fn)
                ...
                # print(grad_norm.dtype, next(model.parameters()).dtype) # float32, float32
                ...
                if iteration % (10 * config.training_config.test_interval) == 0 and rank == 0:
                    file_paths = os.listdir(config.training_config.logdir)
                    for file_path in file_paths:
                        if file_path.endswith("pt") and int(file_path.split("_")[1].replace(".pt", "")) % (10 * config.training_config.test_interval) != 0:
                            os.system(f"rm -rf {os.path.join(config.training_config.logdir, file_path)}")
                if iteration % config.training_config.test_interval == 0 and rank == 0:
                    model.eval()
                    ...
                    with torch.no_grad():
                        ...
                        for i, batch in enumerate(
                            ...
                            mels = mel_fn(batch)
                            test_loss_ = (model if args.n_gpus == 1 else model.module).compute_loss_reflow(mels, batch, train_mode=False, phase_fn=phase_fn,\
                                                                                                           mel_fn=mel_fn)
                        ...
                        for index, test_sample in enumerate(test_batch):
                            ...
                            start = datetime.now()
                            y_0_hat, _ = (model if args.n_gpus == 1 else model.module).forward_reflow(
                        ...
                        if test_l1_spec_loss < min_test_loss:
                            min_test_loss = test_l1_spec_loss
                            best = round(min_test_loss, 3)
